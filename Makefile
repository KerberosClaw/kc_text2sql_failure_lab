...
eval: db     ## run both mocks + refresh gallery data (no API key)
...
eval-model: db  ## score a real model: FLAB_MODEL=... FLAB_API_KEY=... [FLAB_BASE_URL=...] make eval-model
	.venv/bin/python -m failure_lab.runner --provider openai || python3 -m failure_lab.runner --provider openai
	.venv/bin/python -m failure_lab.runner --export-web || python3 -m failure_lab.runner --export-web

eval-cli: db  ## score a local agent CLI: FLAB_CLI_CMD="codex exec" FLAB_MODEL=codex make eval-cli
